...
inline CQuaternion Slerp(const CQuaternion &q1, const CQuaternion &q2, const float t)
{
	...
	if((1.0 - CMath::Abs(dCos)) > DELTA)
	{
		double dTemp = CMath::Acos(CMath::Abs(dCos));
		double dSin = CMath::Sin(dTemp);
		fScale0 = (float)(CMath::Sin((1.0 - t) * dTemp) / dSin);
		fScale1 = (float)(CMath::Sin(t * dTemp) / dSin);
	}
