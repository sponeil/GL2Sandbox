...
public:
	...
	{
		int nX = CMath::Min(m_nWidth-1, CMath::Max(0, (int)(x*(m_nWidth-1)+0.5f)));
		return (void *)((unsigned long)m_pBuffer + m_nElementSize * nX);
	}
	void *operator()(const float x, const float y)
	{
		int nX = CMath::Min(m_nWidth-1, CMath::Max(0, (int)(x*(m_nWidth-1)+0.5f)));
		int nY = CMath::Min(m_nHeight-1, CMath::Max(0, (int)(y*(m_nHeight-1)+0.5f)));
	...
	void *operator()(const float x, const float y, const float z)
	{
		int nX = CMath::Min(m_nWidth-1, CMath::Max(0, (int)(x*(m_nWidth-1)+0.5f)));
		int nY = CMath::Min(m_nHeight-1, CMath::Max(0, (int)(y*(m_nHeight-1)+0.5f)));
		int nZ = CMath::Min(m_nDepth-1, CMath::Max(0, (int)(z*(m_nDepth-1)+0.5f)));
	...
	void Interpolate(float *p, const float x)
	{
		...
		int nX = CMath::Min(m_nWidth-2, CMath::Max(0, (int)fX));
		float fRatioX = fX - nX;
	...
	void Interpolate(float *p, const float x, const float y)
	{
		...
		int nX = CMath::Min(m_nWidth-2, CMath::Max(0, (int)fX));
		int nY = CMath::Min(m_nHeight-2, CMath::Max(0, (int)fY));
		float fRatioX = fX - nX;
		float fRatioY = fY - nY;
	...
	void Interpolate(float *p, const float x, const float y, const float z)
	{
		...
		int nX = CMath::Min(m_nWidth-2, CMath::Max(0, (int)fX));
		int nY = CMath::Min(m_nHeight-2, CMath::Max(0, (int)fY));
		int nZ = CMath::Min(m_nDepth-2, CMath::Max(0, (int)fZ));
		float fRatioX = fX - nX;
		float fRatioY = fY - nY;
		float fRatioZ = fZ - nZ;
...
{
protected:
	unsigned short m_nFormat;			// The format of the pixel data (i.e. GL_LUMINANCE, GL_RGBA)
